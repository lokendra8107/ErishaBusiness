...
namespace ErishaBusiness.ViewComponents
{
    public class HeaderViewComponent : ViewComponent
    {
        ...
        public HeaderViewComponent(IBannerLayoutService bannerLayoutService,
        ...
        public IViewComponentResult Invoke()
        {
            ...
            var cacheKey = "headercategoriesitem" + objItem.CategoryModified;
            if (!_memoryCache.TryGetValue(cacheKey, out List<CategoryDto> objCategory))
            {
                ...
                _memoryCache.Set(cacheKey, objCategory, cacheExpiryOptions);
            }

            var headervoucheritemKey = "headervoucheritem" + objItem.ProductVoucherModified;
            if (!_memoryCache.TryGetValue(headervoucheritemKey, out List<ProductVoucherDto> objVouchers))
            {
                objVouchers = new List<ProductVoucherDto>();
                objVouchers = _voucherService.GetAllVouchersList().Result.ToList();
                var cacheExpiryOptions = new MemoryCacheEntryOptions
                {
                    Priority = CacheItemPriority.Normal
                };
                _memoryCache.Set(headervoucheritemKey, objVouchers, cacheExpiryOptions);
            }

            objHeader.categoriers = objCategory;
            objHeader.Vouchers = objVouchers;
        ...
        public void CheckDeviceInfo()
        {
            ...
            var browserinfo = new BrowserCheckDto()
            {
            ...
            {
                HttpContext.Session.SetString("BrowserDevice", "Mobile");
            }
            else
            {
                HttpContext.Session.SetString("BrowserDevice", "Desktop");
            }
        }
    }
}
